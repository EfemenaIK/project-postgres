...
def process_song_file(cur, filepath):
    ...
    for id, i in df.iterrows():
        # insert song record 
        song_data = (i.song_id, i.title, i.artist_id, i.year, i.duration)
        try:
            cur.execute(song_table_insert, song_data)
        except psycopg2.Error as e:
            print("Error: There was a problem inserting rows: songs")
            print (e)
    
        # insert artist record
        artist_data = (i.artist_id, i.artist_name, i.artist_location, i.artist_latitude, i.artist_longitude)
        try:
            cur.execute(artist_table_insert, artist_data)
        except psycopg2.Error as e:
            print("Error: There was a problem inserting rows: artists")
            print (e)
...
def main():
    try:
        conn = psycopg2.connect(host="localhost", port="5432", database="sparkifydb", user="postgres", password="######")
    except psycopg2.Error as e:
